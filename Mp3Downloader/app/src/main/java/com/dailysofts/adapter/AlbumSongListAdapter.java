...
public class AlbumSongListAdapter extends RecyclerView.Adapter<AlbumSongListAdapter.SongSearchHolder> {

    private JSONArray songSearchList;
    Activity mActivity;

    public AlbumSongListAdapter(JSONArray searchResult, Activity activity){
    ...
    public void onBindViewHolder(SongSearchHolder holder, int position) {
        ...
        String category = (String)item.get(ParserConfig.CATEGORY);
        if(StringUtil.isBlank(category)) {
            holder.songCategory.setVisibility(View.GONE);
        }else {
            holder.songCategory.setText(Html.fromHtml(mActivity.getResources().getString(R.string.categoryBoldTxt,category)));
        }
    }
